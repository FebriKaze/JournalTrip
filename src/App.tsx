...
export default function App() {
  ...
  const loadDrivers = useCallback(async () => {
    ...
    const data = await fetchActiveDrivers(selectedDate, selectedShift);
    setDrivers(data);
    
    if (data.length > 0) {
      const stillInList = data.find(d => d.id === selectedDriverId);
      if (!stillInList && !selectedDriverId) {
        setSelectedDriverId(data[0].id);
      }
    }
    setIsDriversLoading(false);
  }, [selectedDate, selectedShift, selectedDriverId]);
  ...
  const loadData = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const data = await fetchDashboardData(selectedDate, selectedDriverId);
      if (data) {
      ...
      <main className={`flex-1 transition-all duration-300 ${isSidebarCollapsed ? 'md:ml-20' : 'md:ml-64'} pt-20 px-4 md:px-8 pb-8`}>
        <div className="max-w-7xl mx-auto">
          {currentPage === 'dashboard' ? (
            <>
              {selectedDriver && <Header selectedDriver={selectedDriver} />}
