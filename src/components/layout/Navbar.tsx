  ...
  return (
    ...
    `}>
      ...
      <button
        ...
        className="p-2 -mr-1 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors shrink-0 md:hidden"
      >
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-1.5 hover:border-red-400 dark:hover:border-red-500 transition-all cursor-pointer outline-none focus:outline-none focus:ring-0"
              >
              ...
              {/* Shift Options */}
              {isShiftOpen && (
                <div className="absolute top-full right-0 mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl z-200 min-w-36 overflow-hidden">
                  <div className="p-1 space-y-0.5">
                    <button
                      onClick={() => { onShiftChange('Day'); setIsShiftOpen(false); }}
                      className={`flex items-center gap-2.5 px-3 py-2.5 text-left w-full rounded-xl transition-colors text-sm font-bold ${
                        selectedShift === 'Day' 
                          ? 'bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400' 
                          : 'hover:bg-slate-50 dark:hover:bg-slate-700/50 text-slate-700 dark:text-slate-300'
                      }`}
                    >
                      <Sun className={`w-4 h-4 ${selectedShift === 'Day' ? 'text-orange-500' : 'text-slate-400'}`} />
                      Day Shift
                    </button>
                    <button
                    ...
                    >
                      <Moon className={`w-4 h-4 ${selectedShift === 'Night' ? 'text-blue-500' : 'text-slate-400'}`} />
                      Night Shift
                    </button>
                  </div>
                </div>
